...
# https://localhost:8080/cards/4
@cards_bp.route('/<int:card_id>')
def get_card(card_id): # card_id = 4
    stmt = db.select(Card).filter_by(id=card_id) # select * from cards where id=4
    card = db.session.scalar(stmt)
    if card:
        return card_schema.dump(card)
    else:
        return{"error": f"Card id {card_id} doesnt exist. Please try again"}, 404
    
# https://localhost:8080/cards
...
def create_comment(card_id):
    ...
    if card:
        ...
        return {"error": f"Card with id {card_id} doesnt exist"}, 404
